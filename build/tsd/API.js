...
var API = (function () {
    ...
    API.prototype.install = function (selection, options) {
        ...
        return this.core.installer.installFileBulk(files, options.saveToConfig, options.overwriteFiles).then(function (written) {
            if (!written) {
    ...
    API.prototype.reinstall = function (options) {
        var _this = this;
        var res = new InstallResult(options);
        return this.core.installer.reinstallBulk(this.context.config.getInstalled(), options.overwriteFiles).then(function (map) {
            res.written = map;
        }).then(function () {
            if (options.saveToConfig) {
                return _this.core.config.saveConfig();
    ...
    API.prototype.link = function (baseDir) {
        ...
        return linker.scanDefinitions(baseDir).then(function (packages) {
            return Promise.reduce(packages, function (memo, packaged) {
                return manager.addToBundle(_this.context.config.bundle, packaged.definitions, true).then(function (change) {
                    if (change.someAdded()) {
                        memo.push(packaged);
                    }
                    return memo;
